...
import { MODES } from "./agent-system-lite.js";import { holdDraft, cancelDraftTimer } from "./agent-draft-lite.js";
...
// ======== 💬 CHAT MEMORY MANAGEMENT (NEW SYSTEM) ========
...
Return as JSON:
{
  ...
  "attractions": [...]
   ...
   // 🔍 STEP 4: LIVE SEARCH + AI STRUCTURING

const searchQuery = `
Flights from ${departure} to ${destination} on ${departureDate}
Budget hotels in ${destination}
cheap restaurants in ${destination}
...
try {
  if (needsLiveSearch(searchQuery)) {
    console.log("🌐 Performing LIVE search...");
    liveData = await performLiveSearch(searchQuery);
  }
} catch (err) {
  console.error("❌ Live search failed:", err);
}

// 🧠 Convert BOTH prompt + live data into structured JSON
const completion = await openai.chat.completions.create({
  ...
  messages: [
    {
      role: "system",
      content: "You are a budget-conscious travel researcher. Use the live search data AND instructions. Do NOT hallucinate."
    },
    {
      role: "user",
      content: `
...
try {
    console.log(`🖊️ Writing Day ${day} narrative for ${travelerName}...`);
    ...
    const completion = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: `You are a warm, expert travel consultant writing personalized itineraries. 
Write flowing narrative paragraphs — NOT bullet points. 
Use the real data provided. Stay within budget. 
Always return valid JSON with a "content" field containing the narrative.`
        },
        {
          role: "user",
          content: prompt
    ...
    if (!parsed.content) {
      parsed.content = `Hey ${travelerName}, here is your Day ${day} in ${destination}. Your budget of ${sym}${dailyBudget} will cover a wonderful day of experiences. Let me get the detailed plan ready for you shortly.`;
    }

    parsed.content = narrativePolish(parsed.content, travelerName, destination);
    console.log(`✅ Day ${day} narrative written successfully`);
    return parsed;
...
app.get("/api/conversation/:tripId", async (req, res) => {
  try {
    ...
    const convo = getOrCreateConversation(req.params.tripId);
...
app.post("/api/assign-agent", async (req, res) => {
  try {
    ...
    if (!tripId || !agentName) {
      return res.status(400).json({ error: "Missing tripId or agentName" });
    }
    ...
    // 🔥 REAL-TIME EMIT
    if (global.io) {
      global.io.to(tripId).emit("agentAssigned", {
        agentName
      });
    }
...
app.post("/api/agent-reply", async (req, res) => {
  try {
    ...
    // 🔥 REAL-TIME EMIT (ADD THIS LINE)
    if (global.io) {
      global.io.to(tripId).emit("newMessage", {
        role: "agent",
        content: message,
        agentName: agentName || "Agent"
      });
...
// ONLY intercept if firewall gives a DIRECT reply
...
// Allowed → continue
  ...
  // 🚫 STOP AI if agent is handling
  await memoryDB.read(); // ✅ ADD THIS LINE
const convo = memoryDB.data.conversations.find(c => c.tripId === tripId);
...
if (convo?.mode === MODES.TAKEOVER) {
  // 🛑 Cancel any pending AI draft
  if (convo.pendingDraft) {
    cancelDraftTimer(tripId);
    convo.pendingDraft = null;
    await memoryDB.write();
  }

  const reply = `Your travel expert ${convo.assignedAgent || "from SKYmora"} is handling your request. They'll reply shortly. 😊`;
...
if (convo?.mode === MODES.REVIEW && convo?.assignedAgent) {
  ...
  if (result.mode === "plain") {
    aiText = result.text || "";
  } else if (result.mode === "messages") {
    aiText = result.messages?.[0]?.content || "";
  } else if (result.mode === "openai_stream") {
    // 👉 collect stream into buffer (IMPORTANT FIX)
    ...
    for await (const chunk of stream) {
      ...
      if (token) aiText += token;
    }
  }

  if (!aiText) {
    aiText = "Let me help you with that.";
  }

  // ✅ Save draft CORRECTLY
  ...
  // ✅ Send HOLD message to user (VERY IMPORTANT)
  const holdMsg = "Let me check the best options for you and get back shortly! 🔍";
...
io.on("connection", (socket) => {
 ...
 socket.on("joinAgentRoom", (agentName) => {
    socket.join(`agent_${agentName}`);
    console.log(`🧑‍💼 Agent joined room: agent_${agentName}`);
  });

  socket.on("agentTyping", ({ tripId, agentName, isTyping }) => {
    socket.to(tripId).emit("agentTyping", { agentName, isTyping });
  });
